...
int inserir_contato (agenda* A)
{
    ...
    if (A->num_pessoas < A->tam_max)
    {
        ...
        A->c[A->num_pessoas].nome = (char*)malloc (sizeof(char) * (aux+1));
        i = 0;
        while (i<=aux)
        {
        ...
        i=0;
        while (endereco[i-1]!='\0')
        {
            aux = i;
            i++;
        }
        A->c[A->num_pessoas].endereco = (char*)malloc (sizeof(char) * (aux+1));
        i = 0;
        while (i<=aux)
        {
    ...
    {
        printf("\nNao foi possivel adicionar o contato\n");
        return 0;
    }
